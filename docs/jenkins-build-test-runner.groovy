pipeline {
                ...
                script {
                    if (params.FORCE_REBUILD) {
                        sh '''
                            ...
                            docker rmi -f test-runner:latest || true
                        '''
                    }
                }
            }
        ...
        stage('Verify') {
            steps {
                sh '''
                    ...
                    docker run --rm --entrypoint python test-runner:latest --version
                    echo ""
                    echo ">>> Image details:"
                    docker image inspect test-runner:latest --format '{{.Id}} {{.Created}} {{.Size}}'
                '''
            }
        }
    }
    ...
    post {
        success {
            echo "============================================================"
            echo "test-runner:latest built successfully!"
            echo "============================================================"
        }
        failure {
            echo "============================================================"
